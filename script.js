...
(() => {
  ...
  const goSecret = () => {
    // 正規ルートで来たフラグ（前の対策と併用するなら）
    localStorage.setItem("secret_unlocked", "yes");
    location.href = "secret-of-eden.html";
  };

  trigger.addEventListener("click", goSecret);
  // モバイルの反応を良くしたい場合（基本は click だけでOK）
